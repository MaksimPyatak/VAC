      ...
      <QuizQuestion>
         ...
         <template #questionMain>
            <div class="long-income__main">
               <Form class="long-income__form" method="post" :validation-schema="shema">

                  <Field v-slot="{ field, meta, errors }" type="text" name="number" @input="$emit('blockMessageButton')">
                     <input id="years" class="long-income__input" type="text" v-bind="field" placeholder="Years"
                  ...
                  <ErrorMessage class="long-income__error-message error-message" name="number" as="div" />

                  <Field v-slot="{ field, meta, errors }" type="text" name="mounths" @input="$emit('blockMessageButton')">
                     <input id="mounths" class="long-income__input" type="text" v-bind="field" placeholder="Mounths"
                        :class="{ 'input-active': meta.valid && meta.touched, 'input-error': !meta.valid && meta.touched }"
                        @change="selectOption(meta.valid, meta.dirty)" @input="$emit('blockMessageButton')">
                  </Field>
                  <ErrorMessage class="long-income__error-message error-message" name="mounths" as="div" />
               </Form>
            </div>
         </template>
      </QuizQuestion>
